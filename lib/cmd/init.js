...
module.exports = async (name, cmd) => {
  ...
  const input = next
  const ts2jsonPackageJSON = require(`${downloadedT2JTemplatePath}/package.json`)
  const templatePackageJSON = require('../template/package')
  const t2jConfigJSON = require(`${downloadedT2JTemplatePath}/t2j.config`)
  const downloadPackagePath = `${downloadedT2JTemplatePath}/package.json`
  const downloadT2JConfigPath = `${downloadedT2JTemplatePath}/t2j.config.json`
  ...
  templatePackageJSON.dependencies = ts2jsonPackageJSON.dependencies

  fse.outputJsonSync(downloadPackagePath, templatePackageJSON, { spaces: 2 })
  fse.outputJsonSync(downloadT2JConfigPath, t2jConfigJSON, { spaces: 2 })
